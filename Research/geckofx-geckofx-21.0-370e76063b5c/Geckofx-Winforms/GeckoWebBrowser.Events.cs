...
namespace Gecko
{
    public delegate void AutoCompleteEventHandler(GeckoWebBrowser instance,string key,Point point,int width, GeckoElement el);
	partial class GeckoWebBrowser
	{
        public event AutoCompleteEventHandler AutoCompleteCalled;
		#region Event Keys
		// Navigation
		...
		protected virtual void OnDomKeyPress(DomKeyEventArgs e)
		{
            ...
            if (this.Document.ActiveElement.NodeName.ToUpper() == "INPUT")
            {
                GeckoElement el = this.Document.ActiveElement;
                 AutoCompleteCalled(this, (el as Gecko.DOM.GeckoInputElement).Value, PointToClient(MousePosition), (el as Gecko.DOM.GeckoInputElement).ScrollWidth, el);
